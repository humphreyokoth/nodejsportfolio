...
async function sendTestEmail() {
  const to = process.env.CONTACT_NOTIFY_EMAIL || process.env.EMAIL_TO;
  const tx = getTransporter();
  if (!tx) return { ok: false, error: "No SMTP transporter — check SMTP_HOST, SMTP_USER, SMTP_PASS env vars." };
  if (!to) return { ok: false, error: "No recipient — set CONTACT_NOTIFY_EMAIL env var." };

  const senderUser = process.env.SMTP_USER || process.env.EMAIL_USER || "";
  const from = process.env.EMAIL_FROM || `"Humphrey Portfolio" <${senderUser}>`;
  ...
  try {
    const info = await tx.sendMail({
      ...
      text: "This is a test email from your portfolio contact system. If you received this, email is working!",
    });
...
module.exports = { notifyContactForm, getTransporter, sendTestEmail };
